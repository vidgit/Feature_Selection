...
cwd = os.getcwd()
path = cwd + "/Output.txt"      # all possible terms are precomputed like in the moodle files
...
st = string.punctuation                 # for stop word removal


def trim(x):            # This function is called to remove punctuations from either end of word
    x = x.lower()       # '(abcd)' becomes 'abcd'
    if x[0] in st:      # The moodle code did not do this
        x = x[1:]
    if x == '':
        return x
    if x[-1] in st:
...
for i in xrange(0, doc_num[0]):             # All documents are read separately
    ...
    z = [trim(x) for x in t if x != '' and x not in st]                         #stop words are removed after taking lower case
...
for i in xrange(0, doc_num[1]):
    ...
    z = [trim(x) for x in t if x != '' and x not in st]
    for k in z:
        d[k] = d[k] + 1
    doc.append(z)
    doc0.append(z)
...
##################################################################################################################
#############################################TF-BNS Calculation############################################3###

tfdict=dict()                         #We are creating a dictionary where the
tfnum=0                               #key is the stemmed word and value in a unique integer.
for i in pos_dict_words_bns:          #pos_dict_words_bns is a set which contains all the unique stemmed words 
    tfdict[i]=tfnum                   #from all the documents.
    tfnum = tfnum+1

# No. of rows = No. of docs
# No. of columns = No. of features
#tfarray is the term frequency array with rows as docs and columns as features
...
# TPR = No. of docs of class x which contain feature/ Total no. of docs of class x
# FPR = No. of docs of not class x which contain feature / Total no. of docs of not class x

#bnsarray is the array which contains TPR and FPR values for all the documents for each class
bnsarray = np.zeros((len(pos_dict_words_bns),8))    #Empty bnsarray with rows as docs and 4*2 
                                                    # (No. of classes)*(TPR and FPR) columns
# BNS is calculated as |Normal_CDF(TPR) - Normal_CDF(FPR)| 
tpr=0
fpr=0
bns=0
for i in xrange(0,len(pos_dict_words_bns)):
    for j in xrange(0,len(orig_labels)):
        if orig_labels[j]==1 and (pos_dict_words_bns[i] in stemmed_doc[j]):
            tpr +=1                                                        #Calculating TPR
        if orig_labels[j]!=1 and (pos_dict_words_bns[i] in stemmed_doc[j]):
            fpr +=1                                                        #Calculating FPR
    bnsarray[i][0]=tpr/a
    if bnsarray[i][0] == 0:                                    #If tpr/a is zero we assing it 0.0005 because
                                                                # normal_cdf(0) gives infinity as result
        bnsarray[i][0]=0.0005
    
    bnsarray[i][1]=fpr/(a+a+a)                                 #If tpr/a is zero we assing it 0.0005 because
                                                                # normal_cdf(0) gives infinity as result
    if bnsarray[i][1] == 0:
        bnsarray[i][1]=0.0005
    tpr=0
...
for i in xrange(0,len(pos_dict_words_bns)):
    ...
    #finalbnsarray is calculated using weighted average of all the bns values for different classes.
    finalbnsarray[i]=0.25*(norm.ppf(bnsarray[i][0])-norm.ppf(bnsarray[i][1]))+0.25*(norm.ppf(bnsarray[i][2])-norm.ppf(bnsarray[i][3]))+0.25*(norm.ppf(bnsarray[i][4])-norm.ppf(bnsarray[i][5]))+0.25*(norm.ppf(bnsarray[i][6])-norm.ppf(bnsarray[i][7]))

finalbnsarray=np.absolute(finalbnsarray)        #taking modulus of finalbnsarray
j=0
for i in xrange(0,len(finalbnsarray)):
    if finalbnsarray[i]>1:
        finalbnsarray[i]=0.25

#tfbnsmat is the TF-BNS matrix calculated just like TF-IDF using a term frequency matrix and a bns matrix
...
print tfbnsmat.shape[1],vectors_np.shape[1]
################################################################################################################################
...
ig_vectorB=ig_wrapper(tfbnsmat,orig_labels,0.1)
    # The code for Information gain is longer, so it's kept in a different file.
