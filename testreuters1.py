...
doc_num=map(int, np.divide(doc_num,2))
...
cwd = os.getcwd()
path = cwd + "/Reuters_Dataset/Output.txt" 		# all possible terms are precomputed like in the moodle files
...
for i in xrange(0, doc_num[0]):						# All documents are read separately
	name = "/Reuters_Dataset/crude/crude_" + (str)(i)+".txt"
	path = cwd + name
	file = open(path, 'r')
	t = file.read()
...
for i in xrange(0, doc_num[1]):
	name = "/Reuters_Dataset/interest/interest_" + (str)(i)+".txt"
	path = cwd + name
	file = open(path, 'r')
	t = file.read()
...
for i in xrange(0, doc_num[2]):
	name = "/Reuters_Dataset/ship/ship_" + (str)(i)+".txt"
	path = cwd + name
	file = open(path, 'r')
	t = file.read()
	t=filter(lambda w: not w.lower() in s,re.split(regexPattern, t))
...
for i in xrange(0, doc_num[3]):
	name = "/Reuters_Dataset/trade/trade_" + (str)(i)+".txt"
	path = cwd + name
	file = open(path, 'r')
	t = file.read()
	t=filter(lambda w: not w.lower() in s,re.split(regexPattern, t))
...
for i in xrange(0, doc_num[4]):
	name = "/Reuters_Dataset/grain/grain_" + (str)(i)+".txt"
	path = cwd + name
	file = open(path, 'r')
	t = file.read()
	t=filter(lambda w: not w.lower() in s,re.split(regexPattern, t))
...
for i in xrange(0, doc_num[5]):
	name = "/Reuters_Dataset/money-fx/money-fx_" + (str)(i)+".txt"
	path = cwd + name
	file = open(path, 'r')
	t = file.read()
	t=filter(lambda w: not w.lower() in s,re.split(regexPattern, t))
...
print (100*(len(c0)-len(c1))/float(len(c0))), "percent features trimmed by removing words with many meanings"
...
cases=['gnb','mnb','bnb','lin','rbf','poly','rfc','lr','knn']
for i in cases:
    Tester(i)
    print "\n"
